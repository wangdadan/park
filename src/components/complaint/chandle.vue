<template>
  <div class="chandle">
    ...
    <el-table :data="tableData" stripe border :height="tableHeigt" style="width: 100%">
      ...
      <el-table-column width="100" prop="state" label="受理状态" :show-overflow-tooltip="true" resizable>
        <template slot-scope='scope'>
          {{scope.row.state == 0 ? '未受理' : (scope.row.state == 1 ? '已受理' : '')}}
        </template>
      </el-table-column>
      <el-table-column label="操作" width="260">
        <template slot-scope="scope">
          <el-button v-if="scope.row.state==0" title="受理" circle type="success" size="mini" icon="el-icon-check"
                     @click="accept(scope.row)"></el-button>
          <el-button v-if="scope.row.state==1" title="处理" circle type="success" size="mini" icon="el-icon-check"
                     @click="deal(scope.row)"></el-button>
          <el-button v-if="scope.row.state==0" title="拒绝" circle type="danger" size="mini" icon="el-icon-close"
                     @click="refuse(scope.row)"></el-button>
          <el-button v-if="scope.row.state==1" title="结束" circle type="danger" size="mini" icon="el-icon-close"
                     @click="over(scope.row)"></el-button>
          <el-button title="置为无效" circle type="primary" size="mini" icon="el-icon-minus"
                     @click="invalid(scope.row.id)"></el-button>
          <el-button title="查看处理记录" circle type="primary" size="mini" icon="el-icon-search"
                     @click="history(scope.row)"></el-button>
          <el-button title="查看详情" circle type="primary" size="mini" icon="el-icon-view"
  ...
  export default {
    ...
    methods: {
      getData() {
        ...
        self.$http.postForm("/sys/complain/list", data).then(function (res) {
          if (res.data.code == 0) {
      ...
      addSubmit() {
        ...
        self.$refs["addForm"].validate(function (valid) {
          if (valid) {
            self.$http.post("/sys/complain/complain", self.addFormData).then(function (res) {
              if (res.data.code == 0) {
      ...
      invalid(id) {
        let self = this;
        self.$http.post("/sys/complain/invalid", {id: id}).then(function (res) {
          if (res.data.code == 0) {
